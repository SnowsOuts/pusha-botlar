...
.setColor("RANDOM")
.setDescription(`Kişinin eklenen ve alınan tüm rollerine bakmak için \`!rollog @Pusha\` komutunu kullanın \n
**Rol Eklenen kişi**\n ${hedef} - **${hedef.id}** `)
...
.setTimestamp()
client.channels.cache.get(client.channels.cache.find(x => x.name == "rollogchannel").id).wsend(emed)
roller.findOne({
user: hedef.id
}, async (err, res) => {
if (!res) {
...
.setAuthor(hedef.tag, hedef.displayAvatarURL({ dynamic: true }))
.setColor("RANDOM")
.setDescription(`Kişinin alınan ve eklenen tüm rollerine bakmak için \`!rollog @Pusha\` komutunu kullanın \n
**Rolü Alınan kişi** \n${hedef} - **${hedef.id}**`)
.addField(`${red} Rolü Alan Kişi`, `${yapan} - **${yapan.id}**`, false)
.addField(`${red} Alınan Rol`, `${role} - **${role.id}**`, false)
.setFooter(yapan.tag, yapan.displayAvatarURL({ dynamic: true }))
.setTimestamp()
client.channels.cache.get(client.channels.cache.find(x => x.name == "rollogchannel").id).wsend(emeed)
roller.findOne({
user: hedef.id
}, async (err, res) => {
if (!res) {
