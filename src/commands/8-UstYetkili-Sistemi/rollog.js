...
help: "rollog"
  ...
  run: async (client, message, args, embed) => {
        if (!message.member.hasPermission("VIEW_AUDIT_LOG") && !conf.banHammer.some(x => message.member.roles.cache.has(x))) return;
        const Member = message.mentions.members.first() || message.guild.members.cache.get(args[0]) || message.member;
        const Veri = await roller.findOne({ user: Member.id });
        if (!Veri) return message.channel.send(new Discord.MessageEmbed().setDescription(`<@${Member.id}> kişisinin rol bilgisi veritabanında bulunmadı.`, message.author, message.channel))
        let page = 1;
