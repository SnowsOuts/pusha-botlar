...
const { red, green, kirmiziok } = require("../../configs/emojis.json")
module.exports = {
  conf: {
    aliases: ["uyarı","warn"],
    name: "uyarı",
...
${kirmiziok} Ceza ID: \`#${penal.id}\`
${kirmiziok} Uyarılan Üye: ${member.toString()} \`(${member.user.username.replace(/\`/g, "")} - ${member.user.id})\`
${kirmiziok} Uyaran Yetkili: ${message.author} \`(${message.author.username.replace(/\`/g, "")} - ${message.author.id})\`
${kirmiziok} Uyarı Tarihi: \`${moment(Date.now()).format("LLL")}\`
${kirmiziok} Uyarı Sebebi: \`${reason}\`
${kirmiziok} Uyarı Sayısı: \`${uyariData ? Math.floor(parseInt(uyariData.sayi)) : 1} Uyarı\`
      `);
