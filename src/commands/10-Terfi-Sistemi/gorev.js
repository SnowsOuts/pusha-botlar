...
const { galp, Muhabbet, mesaj2, staff, rewards , fill, empty, fillStart, emptyEnd, fillEnd } = require("../../configs/emojis.json")
...
`**İnvite Görev Durumu :** 
${staff} ${progressBar(gorevData ? gorevData.invite : 0, 100, 10)} \`${total} (${total}/100)\` 
` : "";
          //
...
`**Kayıt Görev Durumu :**  
${Muhabbet} ${progressBar(kayitgData ? kayitgData.kayit : 0, 200, 10)} \`${kayittotal} (${kayittotal}/200)\`
` : "";
          //
...
`**Mesaj Görev Durumu :**  
${mesaj2} ${progressBar(mesajData ? mesajData.mesaj : 0, 5000, 10)} \`${mesajtotal} (${mesajtotal}/5000)\`
` : "";
          //
...
`**Taglı Üye Durumu :**  
${galp} ${progressBar(tagData ? tagData.tagli : 0, 50, 10)} \`${tagTotal} (${tagTotal}/50)\`
` : "";


message.channel.send(embed.setDescription(`
${member.toString()}, (${member.roles.highest}) rolüne ait görevlerin aşağıda belirtilmiştir. Görevler tamamlandığında tamamladığın görevlerin ödüllerini otomatik verilmiştir 

Kalan Süre: \`${moment.duration(moment().endOf('week').valueOf() - Date.now()).format("D [gün], H [Saat], m [Dakika] s [Saniye]")}\`
