...
run: async (client, message, args, embed, prefix) => { 
  ...
  let ayar = await serverSettings.findOne({
    ...
    {
    message.react(red)
    message.lineReply(`\`${prefix}isim <@pusha/ID> <Isim> <Yaş>\``).then(x=>x.delete({timeout:5000})) 
    return }
...
.setFooter(`Üyesinin önceki isimlerine <!isimler @pusha/ID> komutuyla bakarak kayıt işlemini gerçekleştirmeniz önerilir.`))
