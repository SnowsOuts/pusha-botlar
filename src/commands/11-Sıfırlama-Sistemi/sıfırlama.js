...
const coins = require("../../schemas/coin");
...
if (!message.member.hasPermission('ADMINISTRATOR') && !conf.banHammer.some(x => message.member.roles.cache.has(x))) 
{
...
return;
    ...
    var DeleteCoin = new MessageButton()
    .setLabel("Coin Sıfırla")
    .setID("coin_sıfırla")
    .setStyle("green")
    ...
    .addComponents(DeleteName, DeleteCoin, DeletePenalty, DeletePenal, Iptal)
...
msg.edit({
  ...
  components : null
      ...
      if(button.id === "coin_sıfırla") {
        await button.reply.defer()
        await coins.deleteMany({userID: member.user.id, guildID: message.guild.id})
      const coinpuan = new MessageEmbed()
      .setDescription(`${green} ${member.toString()} üyesinin Coini ${message.author} tarafından \`${moment(Date.now()).format("LLL")}\` tarihinde temizlendi!`)
